...
echo "🚀 Building TickAggregator (Rust) for Linux version $VERSION ($COMMIT)..."
...
BINARY_NAME="TickAggregator"  # or use jq if you prefer dynamic detection

# Move the compiled exe to output folder
cp "target/release/${BINARY_NAME}" "$OUT_DIR/${BINARY_NAME}-linux"
